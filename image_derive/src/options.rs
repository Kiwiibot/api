...
pub fn derive_options(input: &DeriveInput) -> Result<TokenStream, Error> {
    ...
    let image_options_impl = quote! {
        impl crate::utils::builder::ImageOptions for #name {
            fn to_options(&self) -> Vec<crate::core::types::ImageOption> {
                Vec::from([
                    #(#options),*
                ])
            }
...
fn parse_option(field: &Field) -> Result<ImageOption, Error> {
    ...
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        match self {
            ImageOption::Boolean {
                ...
                tokens.extend(quote! {
                    crate::core::types::ImageOption::Boolean {
            ...
            ImageOption::String {
                ...
                let field_name_str = field_name.unraw().to_string();
                tokens.extend(quote! {
                    crate::core::types::ImageOption::String {
                        name: #field_name_str.to_string(),
                        default: #default,
                        choices: #choices,
                        description: #description,
            ...
            ImageOption::Integer {
                ...
                let field_name_str = field_name.unraw().to_string();
                tokens.extend(quote! {
                    crate::core::types::ImageOption::Integer {
                        name: #field_name_str.to_string(),
                        default: #default,
                        minimum: #minimum,
                        maximum: #maximum,
            ...
            ImageOption::Float {
                ...
                let field_name_str = field_name.unraw().to_string();
                tokens.extend(quote! {
                    crate::core::types::ImageOption::Float {
                        name: #field_name_str.to_string(),
                        default: #default,
                        minimum: #minimum,
                        maximum: #maximum,
...
fn default_value_tokens(options: &Vec<ImageOption>) -> Vec<proc_macro2::TokenStream> {
    options
        .iter()
        .map(|option| {
            if let ImageOption::Boolean {
                field_name,
                default,
                ..
            } = option
            ...
            } else if let ImageOption::String {
                field_name,
                default,
                ..
            } = option
            ...
            } else if let ImageOption::Integer {
                field_name,
                default,
                ..
            } = option
...
fn field_tokens(options: &Vec<ImageOption>) -> Vec<proc_macro2::TokenStream> {
    options
        .iter()
        .map(|option| {
            if let ImageOption::Boolean {
...
fn checker_tokens(options: &Vec<ImageOption>) -> Vec<proc_macro2::TokenStream> {
    options
        .iter()
        .map(|option| {
            if let ImageOption::String {
                field_name,
                choices,
                ..
            } = option
...
fn setter_tokens(options: &Vec<ImageOption>) -> Vec<proc_macro2::TokenStream> {
    options
        .iter()
        .map(|option| {
            if let ImageOption::Boolean { field_name, .. } = option {
                quote! {#field_name: wrapper.#field_name}
            } else if let ImageOption::String { field_name, .. } = option {
                quote! {#field_name: wrapper.#field_name}
            } else if let ImageOption::Integer { field_name, .. } = option {
                quote! {#field_name: wrapper.#field_name}
            } else if let ImageOption::Float { field_name, .. } = option {
                quote! {#field_name: wrapper.#field_name}
            } else {
                unreachable!()
            }
